...
│   ├── Player.kt            # 玩家角色（连招、冲刺、飞刀、受击、祝福增益）
│   ├── Enemy.kt             # 敌人系统（9种敌人类型、状态机AI、Boss机制）
│   ├── Merchant.kt          # 商人 NPC
│   └ Particle.kt            # 粒子特效
├── level/                   # 关卡系统
...
│   ├── Door.kt              # 门（锁定/解锁状态）
├── combat/                  # 战斗系统
│   ├── Projectile.kt        # 投射物（飞刀/魔法弹/火球/矛/宙斯闪电，穿透/爆炸机制）
├── blessing/                # 祝福（Roguelike 增益选择系统）
│   ├── Blessing.kt          # 祝福数据（16种祝福，4类×3稀有度）
│   ├── BlessingSelector.kt  # 祝福选择器（随机3选1）
├── shop/                    # 商店系统
...
- **层级主题**: 每层冥界有独立的配色方案（暗紫/暗红/暗绿）
- **粒子系统**: 受击/死亡/火痕/爆炸产生粒子特效，衰减消亡
