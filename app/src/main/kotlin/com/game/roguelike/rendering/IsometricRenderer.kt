...
class IsometricRenderer {
    ...
    private fun drawIsometricPillar(canvas: Canvas, sx: Float, sy: Float, color: Int) {
        val hw = tileWidth / 2.5f
        val hh = tileHeight / 2.5f
        val height = 28f

        // Base diamond
        paint.color = lighten(color, 1.1f)
        paint.style = Paint.Style.FILL
        canvas.drawPath(Path().apply {
            moveTo(sx - hw, sy)
            lineTo(sx, sy + hh)
        ...
        // Right face
        paint.color = darken(color, 0.75f)
        canvas.drawPath(Path().apply {
            moveTo(sx, sy + hh)
            lineTo(sx + hw, sy)
            lineTo(sx + hw, sy - height)
        ...
        // Top face
        paint.color = lighten(color, 1.4f)
        canvas.drawPath(Path().apply {
            moveTo(sx - hw, sy - height)
            lineTo(sx, sy + hh - height)
            lineTo(sx + hw, sy - height)
        ...
        // Top highlight ring
        paint.color = lighten(color, 1.8f)
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = 2f
        canvas.drawOval(sx - hw * 0.5f, sy - height - hh * 0.8f, sx + hw * 0.5f, sy - height - hh * 0.2f, paint)
        paint.strokeWidth = 1f
        paint.style = Paint.Style.FILL
    }
    ...
    private fun drawIsometricChest(canvas: Canvas, sx: Float, sy: Float) {
        ...
        // Left side
        paint.color = Color.parseColor("#6B3E1C")
        canvas.drawPath(Path().apply {
            moveTo(sx - cw, sy)
            lineTo(sx, sy + ch)
        ...
        // Right side
        paint.color = Color.parseColor("#5B2E0C")
        canvas.drawPath(Path().apply {
            moveTo(sx, sy + ch)
            lineTo(sx + cw, sy)
            lineTo(sx + cw, sy - height)
        ...
        // Top lid
        paint.color = Color.parseColor("#A0724C")
        canvas.drawPath(Path().apply {
        ...
        val lockPulse = (sin(globalTime * 2f) * 0.3f + 0.7f)
        paint.color = Color.argb((255 * lockPulse).toInt(), 255, 215, 0)
        canvas.drawCircle(sx, sy - height - ch * 0.5f, 3f, paint)

        // Sparkle
        paint.color = Color.argb((150 * lockPulse).toInt(), 255, 255, 150)
        canvas.drawCircle(sx, sy - height - ch * 0.5f - 6f, 2f, paint)
    }
    ...
    private fun drawIsometricLava(canvas: Canvas, sx: Float, sy: Float) {
        ...
        val pulse = sin(globalTime * 4f) * 0.3f + 0.7f
        paint.color = Color.argb((180 * pulse).toInt(), 255, 80, 0)
        val inset = 4f
        canvas.drawPath(Path().apply {
        ...
        // Bright center
        paint.color = Color.argb((120 * pulse).toInt(), 255, 160, 0)
        val inset2 = 8f
        canvas.drawPath(Path().apply {
        ...
        // Flame particles
        for (i in 0..2) {
            val fx = sx + sin(globalTime * 3f + i * 2.1f) * 6f
            val fy = sy - 8f + cos(globalTime * 4f + i * 1.7f) * 4f
            val fAlpha = (sin(globalTime * 5f + i) * 0.4f + 0.6f) * 150
            paint.color = Color.argb(fAlpha.toInt(), 255, 200, 50)
    ...
    private fun drawIsometricWater(canvas: Canvas, sx: Float, sy: Float) {
        ...
        paint.color = Color.argb(200, 20, 50, 120)
        paint.style = Paint.Style.FILL
        canvas.drawPath(Path().apply {
            moveTo(sx, sy - hh)
            lineTo(sx + hw, sy)
        ...
        val ripple = sin(globalTime * 2f) * 0.2f + 0.8f
        paint.color = Color.argb((150 * ripple).toInt(), 40, 100, 180)
        val inset = 3f
        canvas.drawPath(Path().apply {
            moveTo(sx, sy - hh + inset)
            lineTo(sx + hw - inset, sy)
        ...
        paint.style = Paint.Style.STROKE
        paint.strokeWidth = 1f
        val rippleOffset = sin(globalTime * 3f) * 4f
        canvas.drawLine(sx - 6f + rippleOffset, sy - 2f, sx + 6f + rippleOffset, sy - 2f, paint)
        canvas.drawLine(sx - 4f - rippleOffset, sy + 2f, sx + 4f - rippleOffset, sy + 2f, paint)
        paint.style = Paint.Style.FILL

        // Light reflections
        val refPulse = sin(globalTime * 1.5f) * 0.3f + 0.5f
        paint.color = Color.argb((60 * refPulse).toInt(), 180, 220, 255)
        canvas.drawCircle(sx - 4f, sy - 3f, 2f, paint)
        canvas.drawCircle(sx + 5f, sy + 1f, 1.5f, paint)
    }

    private fun drawIsometricSpike(canvas: Canvas, sx: Float, sy: Float, color: Int) {
        // Multiple spike points
        val spikeColor = darken(color, 0.5f)
        val spikeHighlight = lighten(color, 1.5f)

        for (i in 0..2) {
            val ox = (i - 1) * 8f
            val spikeHeight = 14f + sin(globalTime * 6f + i * 1.5f).toFloat() * 2f
            ...
            paint.color = spikeHighlight
            paint.style = Paint.Style.STROKE
            paint.strokeWidth = 1f
            canvas.drawLine(sx + ox, sy - spikeHeight, sx + ox + 2f, sy - 2f, paint)
            paint.style = Paint.Style.FILL
        }

        // Danger pulse glow
        val dangerPulse = sin(globalTime * 4f) * 0.3f + 0.7f
        paint.color = Color.argb((30 * dangerPulse).toInt(), 255, 50, 50)
        canvas.drawCircle(sx, sy - 5f, 10f, paint)
    }
